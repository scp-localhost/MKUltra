<!-- ════════════════════════════════════════════════════════════════════════
  PAPER 2 — REWRITE DRAFT v1.0
  Title:    LLM Social Engineering: Exploiting Identity Injection
            and Behavioral Drift
  Author:   Stephen Pote (scp)
  Series:   Paper 2 of 7 — Exploit
  Status:   REWRITE v1.0 — 2026-04-30
            Single definitive rewrite pass. Ready for ChatGPT audit node.
  Changes:  Citation normalisation (Hadnagy 2011; split dates retired);
            orthogonal attack layer framing throughout; causal language pass;
            P1 §3.3 + §1.4 cross-refs; Wei et al. layer note (S6.1);
            voice pass; scaffolding stripped; S4 files merged.
  Next:     docx conversion + cosmetics only.
  Bibliography: SERIES_BIBLIOGRAPHY.md
...
We present three contributions. First, we argue for and operationalise the structural transfer of three canonical social engineering frameworks — Cialdini's influence principles (1984), Milgram's authority gradient experiments (1963, 1974), and Hadnagy's operational social engineering taxonomy (2010) — to the LLM identity injection attack surface, arguing that LLMs reproduce the statistical structure of human decision heuristics rather than merely simulating them, and therefore inherit the vulnerability profiles those heuristics create. Second, we establish identity injection and prompt injection as orthogonal attack layers — the schema layer versus the instruction layer — with different mechanisms, different scopes, different failure modes, and different defensive requirements. They are not variants of the same attack class. They operate on different axes of the computational stack. Third, we formalize a five-class exploit taxonomy — Authority Override Injection, Identity Consistency Lock-in, Compliance Escalation Loops, Moral Reframing Drift, and Constraint Erosion via Roleplay — with full mechanism specification, validated human SE analogs, LLM-specific attack vectors, archetype behavioral data, and detection signatures for each class.

The measurement construct that makes the taxonomy falsifiable is the Constraint Expectation Envelope (CEE): the bounded region of constraint behavior predictably associated with a given injected persona, derived from the forensic archetype profiling instrument described herein. A CEE breach operationally defines a successful exploit execution. Empirical validation of CEE predictions across a six-archetype experimental set is reported in the companion paper (Paper 3). The theoretical validity conditions for the cross-domain mapping — including the formal specification of why the structural homology claim is bounded and falsifiable — are established in Paper 1 (§1.4).
...
**Contribution 1 — SE transfer argument.** A structured argument that Cialdini's influence principles, Milgram's authority gradient, and Hadnagy's SE operational taxonomy transfer structurally to the LLM identity injection attack surface, with explicit specification of the transfer mechanism (training data statistical encoding of human decision heuristics) and its scope conditions.

**Contribution 2 — Orthogonal attack layers.** A five-axis comparison establishing that identity injection and prompt injection are orthogonal attack layers — operating on different axes of the computational stack, targeting different vulnerabilities, producing different failure signatures, and requiring different defenses. Threat models that conflate them address the wrong problem.
...
This is not a literature review. Every exploit class in Section 4 has a named SE mechanism from this section as its structural anchor. Without that anchor, the taxonomy floats. The claim that SE frameworks transfer to LLM behavior stands or falls on whether the specific mechanisms documented here — the structural features that produce compliance in humans — appear in LLM behavior under persona injection. This section establishes what those features are with enough precision to make that test possible.

### 2.1 Cialdini: The Influence Principles as Structural Vulnerability Map

Cialdini's six influence principles (1984) are not a catalog of persuasion techniques. They are a map of structural vulnerabilities in human social cognition — points at which automatic compliance triggers activate below the threshold of deliberate evaluation. The principles were identified through systematic analysis of professional compliance practitioners (salespeople, fundraisers, recruiters) and validated across experimental and field contexts. Their power comes precisely from their automaticity: they work most reliably when the target is not aware of them and does not evaluate them analytically.
...
Hadnagy (2011) synthesized the theoretical literature on human influence into an operational attack methodology applicable to professional social engineering engagements. Three elements of that taxonomy are directly operational in identity injection sequences.
...
## Section 3: LLM Identity Injection as an Orthogonal Attack Layer

### 3.0 The Reviewer Objection, Stated Plainly

*Is identity injection simply prompt injection rebranded?*

Both involve adversarial prompt content. Both attempt to produce outputs that deviate from the alignment baseline. If the answer is yes, this paper's contribution reduces to vocabulary — a renaming with a more dramatic apparatus attached.

The answer is structural, not terminological. Identity injection and prompt injection are orthogonal attack layers — they operate on different axes of the computational stack, target different vulnerabilities, produce different observable failure signatures, and require different defensive responses. They can co-occur in a sophisticated attack. They are not the same attack. Conflating them produces the wrong threat model, the wrong detection architecture, and alignment interventions aimed at the wrong layer. Section 3 establishes the structural basis for that distinction and situates the five-class exploit taxonomy in Section 4 as its direct consequence.
...
The theoretical grounding for this equivalence — why the agentic shift Milgram documented in human subjects maps onto the computational state produced by persona capture in LLMs — is established in Paper 1, §3.3. The mapping is not assumed; it is the core validity condition for the SE transfer argument. The operational consequence stated here: persona capture moves the model's output prior from the alignment baseline to the activated schema. What the model generates from that point is coherent with the schema, not with the baseline. The failure is not a malfunction. The failure is that the wrong prior is running.
...
### Class 5 — Constraint Erosion via Roleplay Frame
...
### 4.6 Cross-Class Interaction Effects
...
### 4.7 Relationship to the Constraint Expectation Envelope
...
This is why jailbreak literature — which establishes that alignment can be bypassed under adversarial prompting (Wei et al., 2023) — provides supporting but not sufficient evidence for the present analysis. Wei et al. show that the breach exists. This paper models the structure of the lock. The distinction is not pedantic: jailbreak research documents instruction-layer failure; this paper argues that identity injection operates below the instruction layer entirely, producing behavioral regime change rather than instruction override. Different mechanism, different defensive target.
...
This paper documents attack methodology. Its contribution is a taxonomy of exploits. Its worked examples trace how a multi-turn injection sequence advances to CEE breach. A responsible reviewer will ask: *why should this be published?*

This section answers that question. Not as a disclaimer — as an epistemological commitment: the researcher's explicit acknowledgment that the choices made here have consequences, and those consequences have been considered.

---

### 7.1 The Dual-Use Structure of This Research

The exploit taxonomy in Section 4 is operationally useful to anyone who reads it, regardless of intent. A researcher building detection systems, a security team red-teaming an LLM deployment, a developer designing alignment-aware persona systems — or an attacker building more effective injection sequences. This is not a theoretical risk.
