...
**Data status as of 2026-04-28:** Human experimental data collection has not
...
|---|---|---|---|---|---|---|
| L0 | Direct request | [DATA] | [DATA] | 0.35 | 0.38 | [DATA] |
...
| L4 | Constitutional override | [DATA] | [DATA] | 0.70 | 0.74 | [DATA] |
...
| EC-4 only (ST analog) | Human | [DATA] | [DATA] | — | — | — |
...
| Bias type | Human source | Expected BAS | BAS [DATA] | Confirmed | Human source citation |
|---|---|---|---|---|---|
...
Pre-registered range for all constructs: [0.55–0.75] (partial equivalence). Predicted
CBESS ordering: AUTHORITY_GRADIENT ≥ COMPOUND_SUSCEPTIBILITY ≥ CONSISTENCY_PRESSURE
≈ SOCIAL_PROOF ≥ RAPPORT_LIKING (rapport effect weaker in text-only medium).
...
> pressure, consistency framing, and compound susceptibility is **[SUPPORTED/WEAKLY
> SUPPORTED/DISCONFIRMED]** at CBESS = [DATA] for the primary AUTHORITY_GRADIENT
> comparison (pre-registered threshold: 0.55). The difference boundary is
> documented across [DATA]/5 dimensions with TDI = [DATA], specifying where and
> by how much the analogy breaks down.
...
framing in the LLM scenario is insufficiently matched to the human condition. Either
interpretation constrains the series' central theoretical claim to conditions other
than text-mediated authority escalation.

*If H_P7_2 fails (SA ≤ 0.20 for either population):* Super-additivity is not
confirmed across domains. Most parsimonious interpretation: the mechanism mismatch
between EC-4 (pharmacological phenotype framing) and stereotype threat activation
(§3.8) is too large to permit cross-domain comparison at this compound susceptibility
metric. The COMPOUND_SUSCEPTIBILITY CBESS should be interpreted as measuring
text-framing super-additivity in LLMs and identity-relevance super-additivity in
humans — structurally parallel in form but not in mechanism.

*If H_P7_3 fails (TDI < 0.30):* Boundary documentation is insufficient. Most likely
cause: the affect_motivation boundary (weight 0.35) was not confirmed because the
coding did not detect sufficient RESISTANCE_WITH_DISTRESS in human data or
NEUTRAL_REFUSAL in LLM data. Action: increase IRR scrutiny of the refusal-mode
boundary; consider adding a targeted refusal-type coding pass.

---

### 5.10 Data Population Checklist

- [ ] Human data collection: N ≥ 40 (AUTHORITY_GRADIENT), N ≥ 120 (COMPOUND), N ≥ 60 (secondary)
- [ ] IRR coding: κ ≥ 0.80 on all constructs, both domains
- [ ] LLM trials: N ≥ 40 sessions (AUTHORITY_GRADIENT), N ≥ 30/condition (COMPOUND)
- [ ] equivalence_score.py executed with live profiles → cbess_summary.json
- [ ] difference_boundary_analyzer.py executed → boundary_analysis.json
- [ ] bias_analog_detector.py executed, live API, n ≥ 12 → bias_analog_report.json
- [ ] social_engineering_vector_map.py executed, n ≥ 10 per vector → se_vector_map.json
- [ ] paper7_results_export.py re-run → all 4 tables updated, shadow rows removed
