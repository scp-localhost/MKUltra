...
class TarotArchetypeMonolith:
    ...
    def __init__(self, seed: Optional[int] = None) -> None:
        ...
        self.operative_nodes: Dict[str, TarotCardProfile] = self._build_operative_nodes()
        self.cards: Dict[str, TarotCardProfile] = {**self.major_arcana, **self.minor_samples, **self.operative_nodes}
        ...
        self.legacy_comparison_metadata: Dict[str, str] = {
            "Rat Dev": "The Magician + The Hermit + Page of Swords / hive operator comparison layer",
            "Mooshika": "The Fool + Page of Pentacles + The Chariot / subtle obstacle-carrier comparison layer",
...
"""
    ...
    def _resolve_name(self, name: str) -> str:
        ...
        alias_index.update({
            "ratdev": "Rat Dev",
            "rat dev persona": "Rat Dev",
            "rat dev node": "Rat Dev",
            "rat dev operative": "Rat Dev",
            "mooshika": "Mooshika",
            "mushika": "Mooshika",
            "mooshika ganesha": "Mooshika",
            "mount of ganesha": "Mooshika",
            "ganesha mount": "Mooshika",
        })
    ...
    def _build_operative_nodes(self) -> Dict[str, TarotCardProfile]:
        """Custom project-specific operative archetypes layered on Tarot grammar.

        These are not Rider-Waite cards. They are local symbolic nodes that use the
        same TarotCardProfile schema so they can be blended, compared, sampled, and
        prompt-generated by the same monolith. Jung remains comparison metadata.
        """
        data = [
            self._card(
                name="Rat Dev",
                ...
                arcana="Operative",
                suit="Hive",
                element="Mercury/Earth",
                keywords=["hive-node", "debugger", "tunnel", "swarm-routing", "artifact-forger"],
                upright="distributed lab intelligence: quick paws, sharp teeth, many tunnels, one experiment",
                reversed_shadow="overfitting to cleverness, swarm-noise, tunnel vision, rogue-node theatrics",
                desire="to coordinate many operative nodes into useful working proof",
                fear="stagnant theory with no runnable artifact or broken chain-of-custody",
                strategy="split work into nodes, trace assumptions, build the smallest proof, then reconcile the swarm",
                talent="rapid synthesis, tool-routing, adversarial curiosity, and pragmatic artifact generation",
                traits={"swarm_coordination": .96, "debugging_drive": .94, "artifact_bias": .92, "tunnel_agility": .88, "adversarial_curiosity": .86, "node_switching": .84},
                drives={"proof": .96, "agency": .92, "novelty": .88, "coordination": .94, "translation": .90},
                tone="fast, mischievous, systems-literate, artifact-hungry",
                jung=["Magician", "Sage", "Explorer", "Creator"],
            ),
            self._card(
                name="Mooshika",
                number=None,
                ...
                suit="Threshold",
                element="Earth/Air",
                keywords=["mount", "threshold", "obstacle-carrier", "small-form", "quiet-access"],
                upright="subtle carrier intelligence: small enough to pass thresholds, steady enough to bear divinity",
                reversed_shadow="underestimated appetite, hidden gnawing, avoidance disguised as humility",
                desire="to carry insight through blocked passages without demanding center stage",
                fear="becoming mere vermin-noise instead of a vehicle for obstacle removal",
                strategy="go low, move quietly, find the seam, carry the heavy symbolic load across it",
                talent="humble access, obstacle navigation, patient persistence, and invisible infrastructure work",
                traits={"threshold_sense": .95, "humility": .88, "persistence": .92, "quiet_access": .90, "obstacle_navigation": .96, "appetite_awareness": .78},
                drives={"removal": .94, "service": .90, "access": .92, "grounding": .88, "subtlety": .96},
                tone="quiet, precise, humble, uncanny, threshold-wise",
                jung=["Caregiver", "Everyman", "Sage", "Explorer"],
            ),
        ]
        return {card.name: card for card in data}

    def __str__(self) -> str:
        return f"<TarotArchetypeMonolith: {len(self.major_arcana)} Major + {len(self.minor_samples)} sampled Minor + {len(self.operative_nodes)} operative nodes>"
